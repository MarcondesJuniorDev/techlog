<div>
    ...
    @if($selectedPermissions)
        <div class="p-4 justify-between sm:space-x-2 space-x-0 sm:space-y-0 space-y-2">
            <div>
                <button wire:click="deleteSelectedPermissions" onclick="confirm('Tem certeza que deseja excluir todos as permissões selecionadas ?') || event.stopImmediatePropagation()" title="Remover permissões selecionadas?" class="w-full flex bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M22 10.5h-6m-2.25-4.125a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0ZM4 19.235v-.11a6.375 6.375 0 0 1 12.75 0v.109A12.318 12.318 0 0 1 10.374 21c-2.331 0-4.512-.645-6.374-1.766Z" />
                    </svg>
                    Deletar Permissões Selecionadas ({{ count($selectedPermissions) }})
                </button>
            </div>
        </div>
    @endif
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            @foreach($permissions as $permission)
                <tr
                    ...
                    <td class="w-4 p-4">
                        <div class="flex items-center">
                            <input type="checkbox" value="{{ $permission->id }}" wire:model.live="selectedPermissions" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    ...
                    <td class="px-6 py-4 text-end">
                        <button wire:click="openEditModal({{ $permission->id }})" class="text-blue-600 hover:text-blue-900" title="Editar {{ $permission->name }}">
                        ...
                        <button wire:click="openViewModal({{ $permission->id }})" class="text-yellow-400 hover:text-yellow-500" title="Detalhes de {{ $permission->name }}">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                                <path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" />
                                <path fill-rule="evenodd" d="M1.323 11.447C2.811 6.976 7.028 3.75 12.001 3.75c4.97 0 9.185 3.223 10.675 7.69.12.362.12.752 0 1.113-1.487 4.471-5.705 7.697-10.677 7.697-4.97 0-9.186-3.223-10.675-7.69a1.762 1.762 0 0 1 0-1.113ZM17.25 12a5.25 5.25 0 1 1-10.5 0 5.25 5.25 0 0 1 10.5 0Z" clip-rule="evenodd" />
                            </svg>
                        </button>
                        <button wire:click="delete({{ $permission->id }})" onclick="confirm('Tem certeza que deseja excluir a permissão {{ $permission->name }}?') || event.stopImmediatePropagation()" class="text-red-600 hover:text-red-900" title="Remover {{ $permission->name }}">
        ...
        <div class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-slate-700 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <form wire:submit.prevent="{{ $editMode ? 'updatePermission' : 'createPermission' }}">
